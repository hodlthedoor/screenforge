...
export async function renderRoutes(
  ...
  async function enqueueRender(
    ...
    if (config.DEDUP_ENABLED) {
      const redis = getDedupRedis(config.REDIS_URL);
      const fingerprint = computeFingerprint({ url, ...options }, type);

      // Try to acquire lock atomically (SET NX)
      ...
      if (lockAcquired === 'OK') {
        ...
        // Poll for the job ID (the winning request might still be inserting)
        let existingJobId: string | null = null;
        for (let i = 0; i < 10; i++) {
          await new Promise(resolve => setTimeout(resolve, 10));
          existingJobId = await checkDedup(redis, fingerprint);
          if (existingJobId && existingJobId !== 'PENDING') break;
        }
      ...
      body: {
        ...
        properties: {
          ...
          actions: {
            type: 'array',
            maxItems: 10,
            description: 'Pre-capture interaction actions to execute after page load and wait strategy, before capture',
            items: {
              type: 'object',
              properties: {
                type: { type: 'string', enum: ['click', 'scroll', 'type', 'hover', 'wait'], description: 'Action type' },
                selector: { type: 'string', description: 'CSS selector (required for click, type, hover; optional for scroll)' },
                value: { type: 'string', description: 'Value (required for type and wait actions; for wait, value is milliseconds as string, capped at 5000)' },
                x: { type: 'number', description: 'X coordinate for scroll (alternative to selector)' },
                y: { type: 'number', description: 'Y coordinate for scroll (alternative to selector)' },
              },
              required: ['type'],
            },
          },
